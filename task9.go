...
func main() {
	...
	admins.Enqueue(&sysadmin{name: "Kolya"})

	// Enqueue two new programmers onto devs.
	devs.Enqueue(&programmer{name: "Ivan"})
	devs.Enqueue(&programmer{name: "Denis"})
	...
	for _, task := range tasks {
		...
		if task.needsAdmin {
			...
			sysadmin := admins.Dequeue()
			sysadmin.administrate(task.system)
			// Dequeue an administrator value from the admins list and
			// call the administrate method.

			continue
		}
		producer := devs.Dequeue()
		producer.develop(task.system)
